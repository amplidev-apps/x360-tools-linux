...
class PythonBridge {
  static String get _bridgeScript {
    ...
    if (Platform.isWindows) {
      // 1. Production Case (MSIX/Bundle): assets/python_backend/service_bridge.py
      // Note: Flutter Windows bundles assets under data/flutter_assets/
      ...
      // 2. Portable/Manual Bundle: Same dir or assets/ subfolder
      final String portablePath = "${exeDir.path}\\assets\\python_backend\\service_bridge.py";
      if (File(portablePath).existsSync()) return portablePath;
      
      return 'service_bridge.py'; // Fallback to current working directory
    }
    ...
    // Debug/Dev Case: Project root
    return '../service_bridge.py';
  }
  
  static String? get _bridgeDir {
    final String script = _bridgeScript;
    if (script.contains(Platform.pathSeparator)) {
      return Directory(script).parent.path;
    }
    return null;
  }

  static String get _pythonCmd {
    if (Platform.isWindows) {
      // Priority: Bundled python.exe -> pythonw.exe -> system python
      final String exePath = Platform.resolvedExecutable;
      final Directory exeDir = File(exePath).parent;
      final String bundled = "${exeDir.path}\\python\\python.exe";
